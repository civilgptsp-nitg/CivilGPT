# app.py — CivilGPT v1.5 (IS-aware refinements)
...
# Positive means MORE water than baseline(angular); negative means LESS.
AGG_SHAPE_WATER_ADJ = {
    "Angular (baseline)": 0.00,     # baseline table assumes angular
    "Sub-angular": -0.03,
    "Sub-rounded": -0.05,
    "Rounded": -0.07,
    "Flaky/Elongated": +0.03,       # penalty
}
...
# IS 383: Fine aggregate grading limits (Zones I–IV) — percent passing ranges
...
# IS 383: Coarse aggregate graded limits (key sieves) — 10, 20, 40 mm nominal sizes (percent passing)
COARSE_LIMITS = {
...
def water_for_slump_and_shape(nom_max_mm: int, slump_mm: int,
    ...
    """
    IS 10262 water estimate:
      - Base from table (angular, 25–50 mm slump)
      - +3% per additional 25 mm slump (without SP)
      - Shape adjustment (rounded needs less, flaky more)
      - SP reduction (15–30%) if enabled
    """
    base = WATER_BASELINE.get(int(nom_max_mm), 186.0)
    # slump adjustment (above 50 mm)
    if slump_mm <= 50:
        water = base
    else:
...
def generate_mix(grade, exposure, nom_max, target_slump, agg_shape,
                 emissions, use_sp=True, sp_reduction=0.18):
    """
    Rule-based optimizer using IS-style durability caps and IS 10262 water target.
    Returns (mix_df, meta)
    """
    w_b_limit = float(EXPOSURE_WB_LIMITS[exposure])
    min_cem = float(EXPOSURE_MIN_CEMENT[exposure])

    # IS 10262 water target based on slump + shape + SP
    ...
    for wb in np.linspace(0.35, w_b_limit, 6):            # candidate w/b ratios
        for flyash_frac in [0.0, 0.2, 0.3]:
            for ggbs_frac in [0.0, 0.3, 0.5]:
                ...
                # Simple aggregate pack (kept constant for stability)
                fine = 650.0
                coarse = 1150.0
                sp = 2.5 if use_sp else 0.0

                mix = {
                    "OPC 43": cement,       # binder base for optimization
                    "Fly Ash": flyash,
                    "GGBS": ggbs,
                    "Water": target_water,
...
def generate_baseline(grade, exposure, nom_max, target_slump, agg_shape,
                      emissions, baseline_type="OPC", use_sp=True, sp_reduction=0.18):
    """Baseline with 100% cementitious as selected cement (OPC 43 or PPC)."""
    w_b_limit = float(EXPOSURE_WB_LIMITS[exposure])
    min_cem = float(EXPOSURE_MIN_CEMENT[exposure])
    ...
    cement_name = "OPC 43" if baseline_type == "OPC" else "PPC"
    mix = {
...
grade = st.sidebar.selectbox("Concrete Grade", list(GRADE_STRENGTH.keys()), index=4)  # default M30
exposure = st.sidebar.selectbox("Exposure Condition", list(EXPOSURE_WB_LIMITS.keys()), index=2)
baseline_choice = st.sidebar.radio("Baseline Mix Type", ["OPC", "PPC"], index=1)
...
st.sidebar.markdown("### Quality Control (for target mean strength)")
...
fa_abs, ca_abs = 1.0, 0.5  # assumed for report only
...
if st.button("Generate Sustainable Mix"):
    if materials_df is None or emissions_df is None:
        ...
        # IS 456: enforce minimum grade by exposure (auto-bump with a warning)
        ...
        if grade_order.index(grade) < grade_order.index(min_grade_required):
            st.warning(f"Exposure **{exposure}** requires minimum grade **{min_grade_required}** by IS 456. "
                       f"Proceeding with {min_grade_required} for calculations.")
        ...
        opt_df, opt_meta = generate_mix(
            ...
            agg_shape=agg_shape, emissions=emissions_df, use_sp=use_sp, sp_reduction=sp_reduction
        )
        base_df, base_meta = generate_baseline(
            grade=grade, exposure=exposure, nom_max=nom_max, target_slump=target_slump,
            agg_shape=agg_shape, emissions=emissions_df, baseline_type=baseline_choice,
        ...
        if opt_df is None or base_df is None:
            ...
            # attach strength meta for reporting
            ...
            with st.expander("Optimized Mix — Assumptions, Strength & Checks", expanded=False):
                c1, c2 = st.columns(2)
                with c1:
                ...
                with c2:
                    ...
                    st.caption("Note: Target mean strength shown per IS 10262. Strength verification requires lab testing.")

            with st.expander(f"{baseline_choice} Baseline — Assumptions, Strength & Checks", expanded=False):
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown("**Derived Parameters**")
            ...
            if fine_csv is None and coarse_csv is None:
                st.info("Upload sieve CSVs (optional) to auto-check IS 383 fine zone & coarse gradation.")
            ...
            fig, ax = plt.subplots()
            ax.bar(["Optimized", f"{baseline_choice} Baseline"], [co2_opt, co2_base])
            ...
            try:
                ...
                story.append(Paragraph(f"Grade: {grade} | Exposure: {exposure} | Baseline: {baseline_choice}", styles["Normal"]))
                ...
                story.append(Paragraph(f"{baseline_choice} Baseline CO₂: {co2_base:.1f} kg/m³", styles["Normal"]))
...
st.markdown("---")
st.caption("CivilGPT v1.5 | IS-aware Sustainable Concrete Mix Designer — Grades M10–M80 • Target Mean Strength • Aggregate Shape • Exposure Grade Rules • IS 383 (10/20/40 mm)")
